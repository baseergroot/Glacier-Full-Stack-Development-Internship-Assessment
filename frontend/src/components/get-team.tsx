...
export default function TeamsCard() {
  ...
  const fetchTeams = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${apiUrl}/admin/teams`, { withCredentials: true });
          ...
          <div key={team._id} className="bg-white rounded-lg shadow-sm border hover:shadow-md transition-shadow">
            <div className="p-6">
              ...
              <div className="mt-4 pt-4 border-t">
                <button onClick={() => navigate(`/team/${team._id}`) } className="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
                  View Team
                </button>
              </div>
            </div>
